...
| [init-writes-opaque-partial-config](./init-writes-opaque-partial-config.md) | enhancement | config, cli | #114 — `pr-triage config show` prints the effective merged config (signal_tiers + routing), read-only |
